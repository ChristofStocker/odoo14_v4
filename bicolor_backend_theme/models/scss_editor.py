...
class ScssEditor(models.AbstractModel):
    ...
    _name = 'bicolor_backend_theme.scss_editor'
    ...
    def _replace_variables(self, content, variables):
        ...
        #content += "\n" + ".o_form_view .oe_button_box .oe_stat_button .o_button_icon {color: %s}" %(variables[1]['value'])
        return content
    ...
    def replace_content(self, url, xmlid, content):
        ...
        if not custom_view.exists():
            ...
            if len(view_to_xpath) > 1:
                for view in view_to_xpath:
    ...
    def _get_custom_attachment_to_remove(self, url,active=True):
        return self.env["ir.attachment"].with_context(
            bin_size=False, bin_size_datas=False
        ).search([("url", '=', url)])
    
    def _get_custom_view_to_remove(self, url,active=True):
        return self.env["ir.ui.view"].search([("name", '=', url), ('active', '=', active)])
        
    def reset_values(self, url, xmlid):
        custom_url = self._get_custom_url(url, xmlid)
        for each in self._get_custom_view_to_remove(custom_url):
            each.write({'inherit_id': False})
        self._get_custom_view_to_remove(custom_url).unlink()
        self._get_custom_view_to_remove(custom_url,False).unlink()
        self._get_custom_attachment_to_remove(custom_url).unlink()
        self._get_custom_attachment_to_remove(custom_url,False).unlink()
